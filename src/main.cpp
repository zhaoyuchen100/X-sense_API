...
void loop_rate_calculate()
{
    ...
    else
    {
        ...
        {
            std::cout<< "sampling rate= :" << sampl_counter/(now.toSec()-begin.toSec())<<std::endl;
            begin = now;
            sampl_counter = 0;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    ros::NodeHandle nh;
    ros::Publisher publisher = nh.advertise<sensor_msgs::Imu> ("orange_imu", 1);

    (void)argc;
    (void)argv;
    int desiredUpdateRate = 100;	// Use 75 Hz update rate for MTWs
        ...
        while (ros::ok() && ! _kbhit()) {
            ...
            for (size_t i = 0; i < mtwCallbacks.size(); ++i) {
                if (mtwCallbacks[i]->dataAvailable()) {
                    ...
                    gyroscope = packet->calibratedGyroscopeData();
            ...
            if (newDataAvailable) {
                ...
                imuData.header.frame_id = "/orange_imu";
